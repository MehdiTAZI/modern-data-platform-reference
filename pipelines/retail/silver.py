...
from mdpr.retail.transforms.customers import latest_customer_state, standardize_customers  # noqa: E402
from mdpr.retail.transforms.orders import add_reference_validity, parse_order_envelope  # noqa: E402
from mdpr.retail.transforms.products import latest_product_state, standardize_products  # noqa: E402
...
def customers():
    checked = annotate_quality(standardize_customers(spark.read.table(f"{CATALOG}.bronze.customers_raw")), CUSTOMERS)
...
def products():
    checked = annotate_quality(standardize_products(spark.read.table(f"{CATALOG}.bronze.products_raw")), PRODUCTS)
...
@dp.table(name="orders_quarantine", comment="Recoverable invalid order events with explicit DQ reasons")
