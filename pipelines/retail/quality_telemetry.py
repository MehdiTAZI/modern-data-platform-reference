...
@dp.materialized_view(
    ...
    comment=(
        "Normalized non-PII quality events emitted from current quarantine surfaces with "
...
def data_quality_events():
    return union_quality_events(
        [
            quality_events(
                spark.read.table("customers_quarantine"),
                CUSTOMERS,
                stage="silver_customer_gate",
            ),
            quality_events(
                spark.read.table("products_quarantine"),
                PRODUCTS,
                stage="silver_product_gate",
            ),
            quality_events(
                spark.read.table("orders_parse_quarantine"),
                ORDERS_INGEST,
            ...
            quality_events(
                spark.read.table("orders_reconciliation_quarantine"),
                ORDERS,
                stage="silver_late_reconciliation",
            ),
            quality_events(
                spark.read.table("orders_reference_reprocess_remaining"),
                ORDERS,
